...
#' 
#' Calculates observational unit-specific scores and corresponding species-specific loadings from a fitted `CBFM` object, potentially at new values of the basis functions. which in turn can be used as a means of spatio-temporal ordination; see Hui et al., (2023) for more details.
#' 
#' @param object An object of class `CBFM`.
#' @param num_comp The number of ordination axes to construct. 
#' @param new_B_space A matrix of new spatial basis functions at which ordinations are to be constructed. If this is not provided, then ordinations based on the original set of basis functions are returned. If at least one of `new_B_space/new_B_time/new_B_spacetime` are provided, then it is \emph{assumed} ordinations at new basis function values are desired.
#' @param new_B_time A matrix of new temporal basis functions at which ordinations are to be constructed. If this is not provided, then ordinations based on the original set of basis functions are returned. If at least one of `new_B_space/new_B_time/new_B_spacetime` are provided, then it is \emph{assumed} ordinations at new basis function values are desired.
#' @param new_B_spacetime A matrix of new spatio-temporal basis functions at which ordinations are to be constructed. If this is not provided, then ordinations based on the original set of basis functions are returned. If at least one of `new_B_space/new_B_time/new_B_spacetime` are provided, then it is \emph{assumed} ordinations at new basis function values are desired.
...
#'
#' Consider now the estimates values of \eqn{b_i^\top a_j} and construct an \eqn{N \times m} matrix of estimated linear predictors with this. We can then construct an ordination from this matrix, albeit in a bit of an ad-hoc manner, by performing a singular value decomposition to obtain matrices of left and and right singular vectors, along with a diagonal matrix of singular values in descending order. The left singular vectors correspond to observational unit-specific scores (similar to latent variables from an latent variable model; see for example Hui et al., 2015, Warton et al., 2015, Niku et al., 2019, van der Veen, 2021) while the right singular vectors correspond to the corresponding species-specific loadings (similar to latent variables from an latent variable model). By taking only the first `num_comp` of the left and right singular vectors and scaling according to the singular values, we can then use these as a means of spatial and/or temporal ordination (Hui et al., 2023)
#'
#' The scores and loadings and thus ordinations can also be constructed at a new set of observational units. This is achieved by supplying the values of `new_B_space/new_B_time/new_B_spacetime` as appropriate, similar to how [predict.CBFM()] functions. Ordinations at new observational units are often desired for spatial temporal ordinations e.g., if the ordination is to be constructed and visualized on a grid of spatial locations, and potentially over time.
#'
