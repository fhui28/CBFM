...
# ##--------------------------------
# ## Zero-truncated Poisson family -- extra functions not available from gamlss
# ## All functions below adapted heavily from and big acknowledgment goes to the authors of the countreg package
# 
# .ztpois_lambda_to_mu <- function(lambda) 
#         ifelse(lambda <= 0, 1, lambda / (1 - exp(-lambda)))
# 
# .make_.ztpois_mu_to_lambda <- function(n = 2000, max = 1000) {
#         ## sampling points (equidistant on log-scale)
#         x <- exp(seq(log(1/n),  log(max), length.out = n))
# 
#         ## set up inverse function
#         f <- stats::splinefun(c(1, .ztpois_lambda_to_mu(x)), c(0, x))
...
#         
#         if(lower.tail) 
#                 rval[p_orig < .dztpois(1, lambda, log = log.p)] <- 1
#         rval
#         }
# 
# .rztpois <- function(n, lambda, mu) {
#         if(!missing(lambda) & !missing(mu)) 
#                 stop("only 'lambda' or 'mu' may be specified")
#         if(!missing(mu)) 
#                 lambda <- .ztpois_mu_to_lambda(mu)
