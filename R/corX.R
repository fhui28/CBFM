...
#' 
#' Takes a fitted `CBFM` object calculates the between-species correlation matrix due to the measured covariates, along with corresponding uncertainty intervals if desired (via a simulation-based approach; see Hui et al., 2023). Similar to [predict.CBFM()], this correlation matrix can be calculated based on a different sets of covariates to those used to actually fit the model. Additionally, the user can supplied two sets of covariates (data frames), in which case the function calculates cross-correlations (between and within species) between these two sets of covariates.
#' 
#' 
#' @param object An object of class `CBFM`.
#' @param newdata A data frame containing the values of the covariates at which correlations are to be calculated. If this is not provided, then correlations corresponding to the original data are returned. If `newdata` is provided then it should contain all the variables needed for constructing correlations, that is, it can construct a model matrix from this as `object$formula`.
#' @param newdata2 A second data frame containing the values of the covariates at which cross-correlations are to be calculated. If this is supplied, then `newdata` must also be supplied, as the function assumes then the user desires calculation of cross-correlations.
