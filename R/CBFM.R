...
#' @param weights A matrix or vector of weights to be applied in association with the \code{y} argument, potentially used to control the contribution of the responses to the log likelihood. If a vector is supplied, then it must be of length \code{nrow(y)}, and this it assumed the same weights applies to all species. Note weights are not internally scaled in any way.
...
#' \item{ridge: }{A ridge parameter that can be included to act as a ridge penalty when estimating the regression coefficients related to the covariates. This can either be a scalar or a vector equal to the number of columns in the model matrix implied by \code{formula} and \code{data} arguments. *Currently, no check is made to ensure the length is compatible with this!*}

#' \item{initial_ziridge: }{A ridge parameter that can be included to act as a ridge penalty when using CBFM's default approach to constructing initial/starting values of the regression coefficients related to the covariates for modeling the zero-inflation probabilities. This can either be a scalar or a vector equal to the number of columns in the model matrix implied by \code{ziformula} and \code{data} arguments. *Currently, no check is made to ensure the length is compatible with this!*}
...
#' \describe{
#' \item{rank: }{The rank of the community-level covariance matrices of the basis function regression coefficients. This either equals to a single scalar/character string equal to "full", or a vector or scalars/character strings (equal to "full") with length equal to how many of \code{B_space/B_time/B_spacetime} are supplied. If it is a single scalar or character string, then it is assumed the same rank is used for all the community-level covariance matrices. 
#' 
...
#' \describe{
#' \item{rank: }{The rank of the between-species correlation matrices of the basis function regression coefficients. This either equals to a single scalar/character string equal to "full", or a vector or scalars/character strings (equal to "full") with length equal to how many of \code{B_space/B_time/B_spacetime} are supplied. If it is a scalar, then it is assumed the same rank is used for all the correlation matrices. 
#' 
...
#' \item{\code{B_time}: }{Both of the approaches mentioned above for \code{B_space} can also be applied here, although with temporal basis functions we have generally found the approach implemented in [FRK::auto_basis()] to work satisfactorily in many cases, given their customizability and sparsity (local support).}
