...
#' 
# Also, this package only implements one possible version of a wider class of CBFMs; other potentially superior versions e.g., spatial basis functions with temporally varying corresponding regressions coefficients, are possible under the CBFM framework, but are far outside the scope of this package (sorry!).      
#'    
...
#' 
#' @details # Warning
#' 1. CBFMs are designed for \emph{spatio-temporal} multivariate abundance data, such that you can sensibly construct basis functions from the space-time coordinate of each observational unit. **Please do not use them for data which are not spatially or temporally indexed**. We recommend you fit standard LVMs in those scenarios e.g., as available in [gllvm::gllvm()], [boral::boral()], and [Hmsc::sampleMcmc()].
#' 
