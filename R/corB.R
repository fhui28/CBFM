...
#' 
#' Takes a fitted `CBFM` object calculates the residual between-species correlation matrix due to (all) the spatial and/temporal basis functions, along with corresponding uncertainty intervals if desired (via a simulation-based approach; see Hui et al., 2023). Similar to [predict.CBFM()], this correlation matrix can be calculated based on a different sets of basis functions to those used to actually fit the model. Additionally, the user can supplied two sets of spatial and/or temporal basis functions, in which case the function calculates cross-correlations (between and within species) between these two sets of basis functions.
#' 
#' 
#' @param object An object of class `CBFM`.
#' @param new_B_space A new matrix of new spatial basis functions at which correlations are to be calculated. If this is not provided, then correlations corresponding to the original `B_space` argument are returned. Please note this should only be supplied if `B_space` was supplied in the original CBFM fit.
#' @param new_B_space2 A second new matrix of new spatial basis functions at which cross-correlations are to be calculated. If this is supplied, then `new_B_space` must also be supplied, as the function assumes then the user desires calculation of cross-correlations due to these spatial basis functions.
#' @param new_B_time A new matrix of temporal basis functions at which calculations are to be calculated. If this is not provided, then correlations corresponding to the original `B_time` argument are returned. Please note this should only be supplied if `B_time` was supplied in the original CBFM fit.
#' @param new_B_time2 A second new matrix of temporal basis functions at which cross-calculations are to be calculated.  If this is supplied, then `new_B_time` must also be supplied, as the function assumes then the user desires calculation of cross-correlations due to these temporal basis functions.
#' @param new_B_spacetime A new matrix of spatio-temporal basis functions at which calculations are to be calculated. If this is not provided, then correlations corresponding to the original `B_spacetime` argument are returned. Please note this should only be supplied if `B_spacetime` was supplied in the original CBFM fit.
#' @param new_B_spacetime2 A second new matrix of spatio-temporal basis functions at which cross-calculations are to be calculated.  If this is supplied, then `new_B_time` must also be supplied, as the function assumes then the user desires calculation of cross-correlations due to these spatio-temporal basis functions.
#' @param se_cor Should uncertainty intervals also be produced for the correlations? Defaults to `FALSE`.
...
#' @param ncores To speed up calculation of the uncertainty estimates, parallelization can be performed, in which case this argument can be used to supply the number of cores to use in the parallelization. Defaults to `detectCores()-1`.
...
#' 
#'
#' @return If `se_cor = FALSE`, then a (cross-)correlation matrix is returned. If `se_cor = TRUE`, then a list with the following components is returned:
...
corB <- function(object, new_B_space = NULL, new_B_space2 = NULL, new_B_time = NULL, new_B_time2 = NULL, new_B_spacetime = NULL, new_B_spacetime2 = NULL, 
                 se_cor = FALSE, coverage = 0.95, ncores = NULL, num_sims = 500) {
        ...
        if(se_cor == TRUE & object$stderrors == FALSE)
