...
#' where \eqn{\ell} is the maximized log-likelihood value (*excluding* the quadratic penalty term in the PQL) of the \code{CBFM} object at convergence \eqn{df} is the (estimated) degrees of freedom (please see [logLik.CBFM()] for more details), \eqn{N} is the number of observational units, and \eqn{m} is the number of species. In light of the use of the maximized log-likelihood value, one may consider that this function constructs something more akin to a conditional AICc; see the discussion in [mgcv::logLik.gam()] and references therein.
#' 
