...
Type objective_function<Type>::operator() () { 
    ...
    DATA_VECTOR(mean_basis_effects_B3); 
    DATA_MATRIX(other_centered_basis_effects_mat_B1);
    DATA_MATRIX(other_centered_basis_effects_mat_B2);
    DATA_MATRIX(other_centered_basis_effects_mat_B3);
    ...
    int num_basisfns_B1 = other_centered_basis_effects_mat_B1.cols();
    int num_basisfns_B2 = other_centered_basis_effects_mat_B2.cols();
    int num_basisfns_B3 = other_centered_basis_effects_mat_B3.cols();
    ...
    for(int k=0; k<num_basisfns_B1; k++) {
          basis_effects_mat_B1(spp_ind - 1, k) = basis_effects(k) - mean_basis_effects_B1(k);
          }
    for(int j=0; j<num_spp; j++) {
          if(j != (spp_ind-1))
              basis_effects_mat_B1.row(j) = other_centered_basis_effects_mat_B1.row(j);
          }
    ...
    for(int k=0; k<num_basisfns_B2; k++) {
          basis_effects_mat_B2(spp_ind - 1, k) = basis_effects(num_basisfns_B1 + k) - mean_basis_effects_B2(k);
          }
    for(int j=0; j<num_spp; j++) {
          if(j != (spp_ind-1))
              basis_effects_mat_B2.row(j) = other_centered_basis_effects_mat_B2.row(j);
          }
    ...
    // Distribution of coefficients for basis functions corresponding to B3 -- see multivariatenormalformatrix.pdf
    matrix<Type> basis_effects_mat_B3(num_spp, num_basisfns_B3);
    for(int k=0; k<num_basisfns_B3; k++) {
        basis_effects_mat_B3(spp_ind - 1, k) = basis_effects(num_basisfns_B1 + num_basisfns_B2 + k) - mean_basis_effects_B3(k);
        }
    for(int j=0; j<num_spp; j++) {
        if(j != (spp_ind-1))
              basis_effects_mat_B3.row(j) = other_centered_basis_effects_mat_B3.row(j);
        }

    matrix<Type> M_B3 = Ginv_B3 * basis_effects_mat_B3 * Sigmainv_B3 * basis_effects_mat_B3.transpose();
    nll += Type(0.5) * M_B3.trace();
    ...
    if(family == 4) { //negative binomial
      for(int i=0; i<num_units; i++) { 
        if(!isNA(y(i))) {
          lik_val(i) = obs_weights(i) * dnbinom2(y(i), exp(eta(i)), exp(eta(i)) + dispparam(0)*pow(exp(eta(i)),2), true);
          }
        if(isNA(y(i))) {
          lik_val(i) = Type(0.0);
          }
