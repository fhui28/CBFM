...
Type objective_function<Type>::operator() () { 
     ...
     DATA_MATRIX(other_basis_effects_mat_B3);
     ...
     int num_basisfns_B3 = other_basis_effects_mat_B3.cols();
     ...
     for(int k=0; k<num_basisfns_B3; k++) {
          basis_effects_mat_B3(spp_ind - 1, k) = basis_effects(num_basisfns_B1 + num_basisfns_B2 + k);
          }
     for(int j=0; j<num_spp; j++) {
          if(j != (spp_ind-1))
               basis_effects_mat_B3.row(j) = other_basis_effects_mat_B3.row(j);
          }
