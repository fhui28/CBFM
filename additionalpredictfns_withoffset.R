...
#' @param new_offset An optional matrix of offsets to be be added when constructing predictions. Note for zero-inflated models, this offset is applied to both the models for mean of the count distribution *and* the probability of zero-inflation.
...
#' @param new_offset_pa For hurdle CBFM models, similar to \code{new_offset} above but applied to the presence-absence component model.
...
#' @param new_offset_count For hurdle CBFM models, similar to \code{new_offset} above but applied to the count component model.
...
predict_with_offsets.CBFM <- function(object, newdata = NULL, manualX = NULL, manualziX = NULL, new_offset = NULL, 
                                      new_B_space = NULL, new_B_time = NULL, new_B_spacetime = NULL, 
     ...
     if(is.null(new_offset))
          new_offset <- Matrix(0, nrow = nrow(new_X), ncol = num_spp, sparse = TRUE)
     ...
     ptpred <- tcrossprod(new_X, object$betas) + tcrossprod(new_B, object$basis_effects_mat) + new_offset
     ptpred <- as.matrix(ptpred)
     colnames(ptpred) <- rownames(object$betas)          
     if(object$family$family[1] %in% c("zipoisson", "zinegative.binomial")) {
          ziptpred <- tcrossprod(new_ziX, object$zibetas) + new_offset
     ...
     if(se_fit) {
          ...
          if(return_internals) {
               ...
               inner_linpredfn <- function(j) {
                    ...
                    linpred <- tcrossprod(new_X, betas_sim) + tcrossprod(new_B, basiseff_sim) + new_offset
          ...
          if(need_sim) {
               ...
               if(object$family$family[1] %in% c("zipoisson","zinegative.binomial")) {
                    innerzip_predfn <- function(j) {
                         ...
                         ptpred <- tcrossprod(new_X, betas_sim) + tcrossprod(new_B, basiseff_sim) + new_offset
                         ziptpred <- tcrossprod(new_ziX, zibetas_sim) + new_offset
                         if(type == "link")
               ...
               if(object$family$family[1] %in% c("ztnegative.binomial") & type == "response") {
                    inner_predfn <- function(j) {
                         ...
                         ptpred <- as.matrix(tcrossprod(new_X, betas_sim) + tcrossprod(new_B, basiseff_sim) + new_offset)
                         ptpred <- exp(ptpred) / (1-dnbinom(0, mu = exp(ptpred), size = matrix(1/object$dispparam, nrow(new_X), num_spp, byrow = TRUE)))
                         return(as.matrix(ptpred))
                         }
...
#' @rdname predict.CBFM
#' @export
predict_with_offsets.CBFM_hurdle <- function(object, 
                                newdata_pa = NULL, manualX_pa = NULL, new_offset_pa = NULL, 
                                new_B_space_pa = NULL, new_B_time_pa = NULL, new_B_spacetime_pa = NULL, 
                                newdata_count = NULL, manualX_count = NULL, new_offset_count = NULL, 
                                new_B_space_count = NULL, new_B_time_count = NULL, new_B_spacetime_count = NULL, 
                                se_fit = FALSE, coverage = 0.95, ncores = NULL, num_sims = 500, ...) {
        if(!inherits(object, "CBFM_hurdle")) 
                stop("`object' is not of class \"CBFM_hurdle\"")
        
        if(!se_fit) {
                preds_pa <- predict.CBFM(object$pa_fit, newdata = newdata_pa, manualX = manualX_pa, new_offset = new_offset_pa,
                ...
                preds_counts <- predict.CBFM(object$count_fit, newdata = newdata_count, manualX = manualX_count, new_offset = new_offset_count,
